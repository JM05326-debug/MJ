...
- 先發投手常常賽前幾小時才公布。鎖定會等到客/主先發都出來才鎖，最遲賽前 10 小時（`FINAL_LOCK_HOURS`）不管有沒有公布都鎖，此時沒公布的就顯示「先發未公布」、模型退化成純 Elo+Poisson（不會因此不預測）。Dashboard 每張賽事卡片會列出客/主先發，看得出來這場有沒有抓到
